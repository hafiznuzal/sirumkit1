...
class Report extends CI_Controller
{
	...
	public function laporan_lainlain($bulan,$tahun,$jenisopt)
	{
		if($this->session->userdata('logged_in'))
		{
			...
			if ($jenisopt == 1) {
				$data['hasil'] = $this->m_report->tabel_bln_lainlain_th_msk($tanggal_lengkap);
 ...
 //   		$this->excel->getActiveSheet()->mergeCells('G'.($numCell+41).':'.'H'.($numCell+41));		
	...
	public function transaksi_lainlain_masuk_excel($bulan,$tahun)
	{
		...
		$this->excel->getActiveSheet()->setCellValue('B1', 'LAPORAN PENERIMAAN LAIN-LAIN ('.$bulan.'-'.$tahun.')');
		$this->excel->getActiveSheet()->setCellValue('B2', 'RUMAH SAKIT UMUM AISYIYAH PADANG');
		...
		$this->load->model('m_report');
		$data= $this->m_report->tabel_bln_lainlain_th_msk($tanggal_lengkap);		
		$temp= $this->m_report->jumlah_lainlain_masuk_perhari($tanggal_lengkap);
		$jumlah =(int)$temp[0]['Jumlah_Id'];

		$numCell=5;
		// print_r($jumlah);
		for ($i=0; $i <$jumlah ; $i++) { 
			$this->excel->getActiveSheet()->setCellValue('B'.$numCell, $i+1);
			
			$this->excel->getActiveSheet()->setCellValue('C'.$numCell,$data[$i]['Uraian']);
	   
			$this->excel->getActiveSheet()->setCellValue('D'.($numCell),$data[$i]['Biaya']);	    	
			$numCell++;
		}
        $this->excel->getActiveSheet()->setCellValue('B'.$numCell,'Jumlah');
         ...
         /** Borders for outside border */
         $BStyle = array('borders' => array('outline' => array('style' => PHPExcel_Style_Border::BORDER_THIN)));
         $this->excel->getActiveSheet()->getStyle('B5'.':'.'B'.($numCell))->applyFromArray($BStyle);	
   		$this->excel->getActiveSheet()->getStyle('B'.($numCell).':'.'D'.($numCell))->applyFromArray($BStyle);
   		$this->excel->getActiveSheet()->getStyle('B'.($numCell))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
   		$this->excel->getActiveSheet()->getStyle('B'.($numCell))->getFont()->setBold(true);
   		$this->excel->getActiveSheet()->getStyle('C5'.':'.'C'.($numCell))->applyFromArray($BStyle);
   		$this->excel->getActiveSheet()->getStyle('D5'.':'.'D'.($numCell))->applyFromArray($BStyle);
   		
   		
        /** Set wrap Text **/
   		$this->excel->getActiveSheet()->getStyle('A1'.':'.'D'.($numCell)) ->getAlignment()->setWrapText(true); 
   		/** Borders for heading */
   		$this->excel->getActiveSheet()->getStyle('B4:D4')->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
   		...
   		$this->excel->getActiveSheet()->setCellValue('A'.($numCell+2), 'RSU AISYIYAH PADANG');
		$this->excel->getActiveSheet()->setCellValue('C'.($numCell+3), 'DIREKTUR');
		$this->excel->getActiveSheet()->setCellValue('D'.($numCell+3), 'KABAG KEUANGAN');
		$this->excel->getActiveSheet()->setCellValue('A'.($numCell+8), 'dr. Hadril Busudin, Sp.S, MHA');
		$this->excel->getActiveSheet()->setCellValue('D'.($numCell+8), 'Bachtar, S.Sos');	
   		$this->excel->getActiveSheet()->mergeCells('A'.($numCell+2).':'.'D'.($numCell+2));
   		// $this->excel->getActiveSheet()->mergeCells('A'.($numCell+3).':'.'C'.($numCell+3));
		// $this->excel->getActiveSheet()->mergeCells('B'.($numCell+46).':'.'H'.($numCell+46));
		// $this->excel->getActiveSheet()->mergeCells('C'.($numCell+52).':'.'D'.($numCell+52));
  		$this->excel->getActiveSheet()->getStyle('D'.($numCell+8))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$this->excel->getActiveSheet()->getStyle('D'.($numCell+3))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$this->excel->getActiveSheet()->getStyle('A'.($numCell+2))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);

		$filename='Laporan Penerimaan Lain Lain'.$tanggal_lengkap.'.xls'; //save our workbook as this file name
		header('Content-Type: application/vnd.ms-excel'); //mime type
		header('Content-Disposition: attachment;filename="'.$filename.'"'); //tell browser what's the file name
		header('Cache-Control: max-age=0');
		$objWriter = PHPExcel_IOFactory::createWriter($this->excel, 'Excel5');
	...
	public function transaksi_lainlain_keluar_excel($bulan,$tahun)
	{
		if ($bulan < 10) {
			$bulan = '0'.$bulan;
		}
		...
		$this->excel->getActiveSheet()->setCellValue('B1', 'LAPORAN PENGELUARAN LAIN-LAIN ('.$bulan.'-'.$tahun.')');
		$this->excel->getActiveSheet()->setCellValue('B2', 'RUMAH SAKIT UMUM AISYIYAH PADANG');
		...
		$this->load->model('m_report');
		$data= $this->m_report->tabel_bln_lainlain_th_klr($tanggal_lengkap);		
		$temp= $this->m_report->jumlah_lainlain_keluar_perhari($tanggal_lengkap);
		$jumlah =(int)$temp[0]['Jumlah_Id'];
		$jumlah_pengeluaran = (int) $temp[0]['Jumlah_Biaya'];
        ...
        $this->excel->getActiveSheet()->setCellValue('B'.$numCell,'Jumlah');
        $this->excel->getActiveSheet()->setCellValue('D'.$numCell,$jumlah_pengeluaran);
		...
		$filename='Laporan Pengeluaran Lain Lain'.$tanggal_lengkap.'.xls'; //save our workbook as this file name
		header('Content-Type: application/vnd.ms-excel'); //mime type
		header('Content-Disposition: attachment;filename="'.$filename.'"'); //tell browser what's the file name
		header('Cache-Control: max-age=0');
		$objWriter = PHPExcel_IOFactory::createWriter($this->excel, 'Excel5');
